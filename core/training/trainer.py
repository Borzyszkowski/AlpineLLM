...
class Trainer:
    ...
    def fit(self):
        ...
        global_iter = 1
        while global_iter <= self.cfg.n_iters:
    ...
    def train(self, global_iter, ds_name='train'):
        ...
        for curr_iter in range(1, self.cfg.val_every_iteration + 1):

            # Get the next batch of data (or restart if iterator is exhausted)
            input_tensor, target_tensor = self.get_next_training_batch()
            ...
            if curr_iter == 1 and global_iter == 1:
    ...
    def get_next_training_batch(self):
        """ Retrieves the next batch from the infinite training dataset iterator """
        try:
