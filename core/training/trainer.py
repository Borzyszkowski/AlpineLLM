...
class Trainer:
    def __init__(self, cfg, hyperparam_cfg, inference_only=False):
        ...
        self.ds_train, self.ds_val, self.ds_test = self.load_data()

    def fit(self):
        """ Main logic, in which the model is trained and evaluated """
        start_time = datetime.now().replace(microsecond=0)
        logging.info(f'Started training at {start_time} for {self.cfg.n_iters} iterations\n')
        ...
        curr_iter = 1
        while True:
            logging.info('--- Current Train/Val Iter # %03d' % curr_iter)

            # Run main training and evaluation logic
            train_loss, curr_iter = self.train(curr_iter)
            val_loss = self.evaluate(curr_iter)
            ...
            with torch.no_grad():
                ...
                self.swriter.add_scalars('total_loss', {'train_loss': train_loss, 'val_loss': val_loss}, curr_iter)
    ...
    def train(self, iter_num, ds_name='train'):
        ...
        train_iter = iter(self.ds_train)
        for curr_iter in range(iter_num, self.cfg.n_iters + 1):

            # Get the next batch of data (or restart if iterator is exhausted)
            try:
                input_tensor, target_tensor = next(self.ds_train)
    ...
    def evaluate(self, iter_num, ds_name='val'):
        ...
        with torch.no_grad():
            for (curr_iter, batch) in enumerate(data, 0):
    ...
    def load_data(self):
        ...
        train_val_kwargs = {'num_workers': self.cfg.n_workers,
